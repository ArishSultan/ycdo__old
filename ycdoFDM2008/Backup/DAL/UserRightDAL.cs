        ...
        public bool SaveUserRights(List<UserRight> userRights)
        {
            try
            {
                ...
                foreach (UserRight item in userRights)
                {
                    if (item.RightId > 0)
                    {
                        cmd.CommandText = "Update UserRight set CanAccess=" + item.CanAccess + " where RightID=" + item.RightId;
                        
                    }
                    else
                    {
                        cmd.CommandText = "Insert into  UserRight (UserID,ScreenId,CanAccess) Values(" + userRights[0].User.Userno + "," + item.Screen.ScreenId + "," + item.CanAccess + ")";
