...
namespace FDM
{
    public partial class frmDoctorCheckup : Form
    {
        ...
        private void LoadRoomTokens()
        {
            ...
            dgvTokens.Columns["TokenDate"].DefaultCellStyle.Format = "dd/MM/yyyy";
            //dgvTokens.Sort(dgvTokens.Columns["TokenNumber"], ListSortDirection.Ascending);
            //dgvTokens.Sort(this.dgvTokens.Columns["TokenDate"], ListSortDirection.Descending);
        ...
        public string name, rname, dname;
        ...
        private void frmDoctorCheckup_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                string[] room = fdcr.frmname.Split('-');
                name = an[0] + " " +an[1];
                rname = room[0].Trim();
                dname = room[1].Trim();
                fr.s = name;
                ...
                if (fdcr.show)
                {
                    ...
                    LoadRoomTokens();
                    //load medicines
                    labtests = new LabTestBLL().GetAvailabeMedicines();
        ...
        private void PopulatePatientDetails(PatientRegistration pr)
        {
            ...
            this.txtPatientDetails.Text += "Age:" + pr.Patient.Age + Environment.NewLine;
            this.txtPatientDetails.Text += "Address :" + pr.Patient.Address + Environment.NewLine;
            this.txtPatientDetails.Text += "Mobile :" + pr.Patient.Mobile + Environment.NewLine;
            this.txtPatientDetails.Text += "Type :" + pr.TokenType.ToString() + Environment.NewLine;

            pr.Room.Name = rname;
            pr.Room.LabelName = dname;
        }
        ...
        {
            if (MessageBox.Show("Do you want to send LabTest to Reception", "See Next", MessageBoxButtons.YesNo) == DialogResult.No)
                return;
            ...
            if (ii_chk==0)                                      //// 0 -> if called directly
            { 
                LoadRoomTokens();
                NextTokenData();
            }
        ...
        {
            frmPatientAdmission frm = new frmPatientAdmission();
            frm.IsLoginUser = IsLoginUser;
            frm.ShowDialog();
        }
