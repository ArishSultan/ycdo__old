            ...
            try
            {
                ...
                if (this.con.ConnectionString != "")
                {
                    ...
                    if (this.con.State == ConnectionState.Open)
                    {
                        ...
                        if (user.IsAdmin)
                        {
                            this.cmd.CommandText = "Select count(uno) from unames where isadmin =1";
                        }
                        else
                        {
        ...
        public bool VerifyUser(User user)
        {
            ...
            try
            {
                ...
                if (this.con.ConnectionString != "")
                {
                    ...
                    {
                        this.cmd.Connection = this.con;
                        this.cmd.CommandText = "Select * from unames where UName='" + user.UserName + "' and Upwd='" + Common.DataBasePassword.Encrypt(user.UserPassword) + "'";
                        this.da = new OleDbDataAdapter(this.cmd.CommandText, this.con);
                        this.da.Fill(this.dt);
                        this.con.Close();
                    }
