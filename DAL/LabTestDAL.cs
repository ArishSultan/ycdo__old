...
namespace DAL
{
    public class LabTestDAL
    {
        ...
        public List<LabTest> GetLabTest()
        {
            ...
            try
            {
                string select = "Select * from LabTest where IsMedicine=0";
                DataTable dt = new DataTable();
                con = new OleDbConnection();
                readconfile = new ReadConfigFile();
                con.ConnectionString = readconfile.ConfigString(ConfigFiles.ProjectConfigFile);
        ...
        public List<LabTest> GetMedicines()
        {
            ...
            try
            {
                string select = "Select * from LabTest where IsMedicine=1";
                DataTable dt = new DataTable();
                con = new OleDbConnection();
                readconfile = new ReadConfigFile();
                con.ConnectionString = readconfile.ConfigString(ConfigFiles.ProjectConfigFile);
        ...
        public bool SaveRecieveMedicine(RecieveMedicine rm)
        {
            try
            {
                ...
                foreach (RecieveLineItem item in rm.Lines)
                {
                    ...
                    {
                        cmdSave.CommandText = "Insert into ReceiveMedicine(RecieveDate,RecievedNumber,ItemNumber,ItemName,Qty,Price,GrossAmount,NetAmount, BranchID) values ('" +
                                             rm.RecieveDate.ToString("MM/dd/yyyy") + "'," + rm.RecieveNumber + "," + item.LineItem.LabTestId + ",'" + item.LineItem.TestName + "'," + 
                                             item.Quantity + "," + item.Price + "," + item.GrossAmount + "," + rm.RefRec.NetAmount + "," + rm.RefBranch.BranchID + ")";
                    }
        ...
        public DSIssueBill.DTIssueBillDataTable PrintRecMedBill(RecieveMedicine Obj)
        {
            ...
            try
            {
                ...
                string Qry = "select RecievedNumber as IssueNumber,RecieveDate as IssueDate,ItemName as MedicineName,Qty as Quantity,Price as Price,GrossAmount,NetAmount, br.BranchName from ReceiveMedicine,LabTest lt, Branches br" +
                             " where lt.ID=ReceiveMedicine.ItemNumber and br.ID=ReceiveMedicine.BranchID and ReceiveMedicine.RecievedNumber=" + Obj.RecieveNumber;
     ...
     " group by  RecieveDate,RecievedNumber,ItemNumber,ItemName) as Final";
        ...
        public DSIssuedMedPur.dtIssuedMedPurDataTable GetIssuedMedPur(bool All,bool Purchase,bool Retail,LabTest MedID,DateTime From,DateTime To )
        {
            ...
            try
            {
                string Pur = "select ilt.TokenDate as IssueDate, ilt.LabTestID as MedicineID,lt.[Test (A-Z)]  as MedicineName,lt.PurchasePrice as PurchasePrice,'' as BranchName,ilt.DosePerDay*ilt.Days as Quantity from InjectionLabTest ilt,LabTest lt where ilt.LabTestID=lt.ID and lt.ID=" + MedID.LabTestId +
                               " and ilt.TokenDate>='"+From+"' and ilt.TokenDate<='"+To+"'  union all" +
                                " select mi.TokenDate as IsssueDate,mi.ID as MedicineID,lt.[Test (A-Z)] as MedicineName,lt.PurchasePrice as PurchasePrice,'' as BranchName,mi.DosePerDay as Quantity  from MedicineIssued mi,LabTest lt where mi.ID=lt.ID and mi.MedicinesIssued=1 and" +
                ...
                " and ilt.TokenDate>='" + From + "' and ilt.TokenDate<='" + To + "' union all" +
                " select mi.TokenDate as IssueDate, mi.ID as MedicineID,lt.[Test (A-Z)] as MedicineName,lt.RetailPrice as RetailPrice,''as BranchName,mi.DosePerDay as Quantity  from MedicineIssued mi,LabTest lt where mi.ID=lt.ID and mi.MedicinesIssued=1 and" +
                " lt.ID=" + MedID.LabTestId +
               " and mi.TokenDate>='" + From + "' and mi.TokenDate<='" + To + "' union all" +
            ...
            //  " union all" +
            //  " select mi.TokenDate as IssueDate, mi.ID as MedicineID,lt.[Test (A-Z)] as MedicineName,lt.RetailPrice as Price,mi.DosePerDay as Quantity  from MedicineIssued mi,LabTest lt where mi.ID=lt.ID and mi.MedicinesIssued=1" +
            // " union all" +
            //" select im.IssueDate as IssueDate,im.MedicineID as MedicineID,im.MedicineName,lt.RetailPrice as Price,im.Qty as Quantity from IssuedMedicine im,LabTest lt where im.MedicineID=lt.ID";
               ...
               " and ilt.TokenDate>='" + From + "' and ilt.TokenDate<='" + To + "' union all" +
               " select mi.TokenDate as IssueDate, mi.ID as MedicineID,lt.[Test (A-Z)] as MedicineName,lt.PurchasePrice as PurchasePrice,lt.RetailPrice as RetailPrice,'' as Branchname,mi.DosePerDay as Quantity  from MedicineIssued mi,LabTest lt where mi.ID=lt.ID and mi.MedicinesIssued=1" +
              " and mi.Tokendate>='" + From + "' and mi.TokenDate<='" + To + "' union all" +
        ...
        public DSMedicines GetListOfMed()
        {
            ...
            try
            {
                ...
                con.ConnectionString = readconfile.ConfigString(ConfigFiles.ProjectConfigFile);
                con.Open();
                string Pur = "select lt.ID as MedicineID, lt.[Test (A-Z)] as  MedicineName from LabTest lt where lt.IsMedicine=1";
                if (con.State == ConnectionState.Open)
                {
